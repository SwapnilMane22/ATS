 ...
 */
function escapeLatexDollars(text: string): string {
 ...
 */
function escapeLatexAmpersand(text: string): string {
  return text.replace(/(?<!\\)&/g, "\\&");
}
 ...
 */
function sanitizeLatexRewrite(text: string): string {
  let s = text;
 ...
 */
function stripLatexForMatch(s: string): string {
  return s
    .replace(/\\newline\b/g, " ")
    .replace(/\\[a-zA-Z]+\{[^}]*\}/g, (m) => {
...
export function applyLatexPatch(
  ...
  const textToLineIndex = new Map<string, number>();
  const strippedToLineIndex = new Map<string, number>();
  for (let i = 0; i < lines.length; i += 1) {
    const raw = lines[i] ?? "";
    ...
    if (itemMatch) {
      ...
      if (norm) textToLineIndex.set(norm, i);
      if (stripped && stripped !== norm) strippedToLineIndex.set(stripped, i);
      // Also index the full raw content (with suffix) in case of partial matches
      const normFull = normalizeWhitespace(rawContent);
      if (normFull && normFull !== norm) textToLineIndex.set(normFull, i);
    }
  }
