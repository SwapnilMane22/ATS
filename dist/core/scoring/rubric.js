...
function containsWeakVerb(text) {
    return /\b(responsible for|worked on|helped with|assisted with|participated in)\b/i.test(text);
}
function containsMetric(text) {
    return /(\b\d+(\.\d+)?\b|%|\$\b|ms\b|s\b|x\b)/i.test(text);
}
function startsWithStrongVerb(text) {
 ...
 */
export function scoreResumeDeterministic(resume) {
    const gates = [];
    ...
    for (const b of resume.bullets) {
        if (startsWithStrongVerb(b.text))
            strongVerbCount += 1;
        if (containsMetric(b.text))
            metricCount += 1;
        if (containsWeakVerb(b.text))
            weakVerbCount += 1;
    }
